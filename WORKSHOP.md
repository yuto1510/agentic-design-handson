...
決めるのは処理のの境界。JSON の形・関数名・ファイル構成は Claude に任せる。
...
コミットしておくと、Step 5 で `git checkout deterministic` したときに自分のファイルが混ざらない
（untracked のままだとブランチを切り替えても付いてくる）。

確認:

- 指定した責務分担どおりになっているか（Claude が境界を動かしていないか）
...
Step 4 で自分の設計を測ったのと同じスクリプト。参考値:
...
**数字は参考値。** エージェントがインターフェースをどこまで探索するかで毎回変わる。
見るのは絶対値ではなく、自分の Step 4 の数字と並べたときの向き。
